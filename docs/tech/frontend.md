...
- 运行时较轻：不使用虚拟 DOM diff，但编译产物仍包含更新 DOM 所需的运行时代码
- 编译优先：更多框架工作在构建阶段完成，而不是“完全没有框架代码”
...
| SSG | ✅ | ✅ | ✅ | ✅ | ✅ |
| SSR | ✅ | ✅ | ✅ | ✅ | ✅ |
| 增量/按需再生成 | ✅（实现依路由模式而异） | ✅（依部署适配器） | 依部署平台与缓存策略 | — | — |
| 流式 SSR | ✅ | ✅ | ✅ | ✅ | — |
| 边缘渲染 | ✅ (Edge Runtime) | ✅ | ✅ | ✅ | — |
| 文件路由 | ✅ App Router | ✅ | ✅ | ✅ (文件系统路由) | — |
| API Routes | ✅ | ✅ | ✅ | ✅ (loader/action) | — |
| 图片优化 | ✅ (next/image) | ✅ | 需适配器或第三方工具 | — | — |
...
框架体积和渲染性能不能用一个固定数字或总排名表示。结果会随框架版本、编译模式、路由和水合策略、应用代码以及测量设备改变。选型时应使用同一业务场景的生产构建，分别测量首屏 JavaScript、LCP、INP、内存占用和服务端成本；公开微基准只能作为线索，不能替代项目级测试。
